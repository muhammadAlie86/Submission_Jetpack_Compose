package com.submission.submissionjetpackcompose.utils.mvvm
...
sealed class DataState<out T> {
    ...
    data class Empty(val empty: Throwable) : DataState<Nothing>()
    ...
    override fun toString(): String {
        return when (this) {
            ...
            is Empty -> "Empty[exception=$empty]"
        }
    }
...
inline fun <T : Any> Flow<DataState<T>>.onEmptyState(crossinline action: suspend (error: Throwable) -> Unit): Flow<DataState<T>> =
    onEach {
        if (it is DataState.Empty) action(it.empty)
    }
